...
print("Count of letters: " + str(counter))
n = sum(counter.values()) * 1.0   # Convert to float so division returns float.
print("Number of values: " + str(n))
print("Frequency of letters: " + str([(char, count / n) for char, count in counter.most_common()]))
...
def generate_corpus():
  ...
  if nltk.download('brown'):
    ...
    for sentence in corpus.sents():
      ...
      encoded = b''.join([word.encode('UTF8').upper() for word in sentence])
      for char in encoded:
