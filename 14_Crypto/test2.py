...
#try:
num = len(rest) - len(shift_key)
list_k=[]
for m in range(0,num):
  for n in range(0,len(shift_key)):
    list_k.append(shift_key[n])
print list_k
#str2='hilwmkbdpcvazusjgrynqxofte'
for k in range(0,len(list_k)):
      rot_string = rightrotate(str1, list_k[k])
      print("Rotated round " + str(k+1) + " is: " + rot_string)  
      final_list= [rot_string]
      convert=str(final_list[0])
      lmap=list(convert)
      dictionary = dict(zip(list_alphabets, lmap))
      str_from_res= str(rest[k])
      print(str_from_res)
      enc_part=subst_cipher(dictionary,str_from_res.replace(" ",""))
      print("Encrypted part " + str(k+2) + " is: " + enc_part + "\n")
      enc_str+=enc_part + " "
      str1= rot_string
      print k
#except:
